<!--
  Narsing1727 — GitHub Profile README
-->

<div align="center">
  <h1>Narsing Sharma</h1>

  <p>
    <img src="https://img.shields.io/badge/LeetCode-Knight-black?style=flat-square&logo=leetcode&logoColor=white" />
    <img src="https://img.shields.io/badge/Codeforces-~1500-black?style=flat-square&logo=codeforces&logoColor=white" />
  
  </p>
</div>

---

Building scalable simulation platforms for real-world disaster scenarios.
Backend-focused engineer · System design · Problem solver.

---

### Projects

<table>
  <tr>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/Narsing1727/Nerolith_Cortex">Nerolith_Cortex</a></h4>
      <p>AI-powered disaster simulation agent for flood modeling. 
.</p>
      <img src="https://img.shields.io/badge/Python-black?style=flat-square&logo=python&logoColor=white" />
      <img src="https://img.shields.io/badge/status-active-black?style=flat-square" />
    </td>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/Narsing1727/Flood-Forge">Flood-Forge</a></h4>
      <p>Hydrology algorithm engine DLL. D8 flow direction, Wang-Liu pit filling, Diffusive Wave SWE, Green-Ampt infiltration, Manning's equation, TWI, topological sort accumulation.</p>
      <img src="https://img.shields.io/badge/C++-black?style=flat-square&logo=cplusplus&logoColor=white" />
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4><a href="https://github.com/Narsing1727/Distributed-Rate-Limiter">Distributed-Rate-Limiter</a></h4>
      <p>Production-grade rate limiting infrastructure. Distributed systems architecture with Redis, sliding window and token bucket algorithms.</p>
      <img src="https://img.shields.io/badge/JavaScript-black?style=flat-square&logo=javascript&logoColor=white" />
    </td>
    <td width="50%" valign="top">
     <h4><a href="https://github.com/Narsing1727/Vault">Vault</a></h4>
<p>Scalable backend ledger system for managing company deposits and transactions. Designed for consistency, concurrency control, and reliable transaction processing in distributed environments.</p>
<img src="https://img.shields.io/badge/JavaScript-black?style=flat-square&logo=javascript&logoColor=white" />
    </td>
  </tr>
</table>

---

### Stack

![C++](https://img.shields.io/badge/C++-black?style=flat-square&logo=cplusplus&logoColor=white)
![Python](https://img.shields.io/badge/Python-black?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=flat-square&logo=javascript&logoColor=white)
![Qt6](https://img.shields.io/badge/Qt6-black?style=flat-square&logo=qt&logoColor=white)
![OpenGL](https://img.shields.io/badge/OpenGL-black?style=flat-square&logo=opengl&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-black?style=flat-square&logo=redis&logoColor=white)
![Kafka](https://img.shields.io/badge/Kafka-black?style=flat-square&logo=apachekafka&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-black?style=flat-square&logo=docker&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-black?style=flat-square&logo=microsoftazure&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-black?style=flat-square&logo=nextdotjs&logoColor=white)

---

